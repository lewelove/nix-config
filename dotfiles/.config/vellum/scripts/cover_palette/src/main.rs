...
fn main() -> Result<()> {
    ...
    let library_root_str = payload
        .pointer("/config/storage/library_root")
    ...
    let img = image::open(&cover_path).context("Failed to open cover image")?;

    let config_path = expand_path("~/.config/vellum/scripts/cover_palette/config.toml");
    let config: ScriptConfig = if config_path.exists() {
    ...
    if let Some(palette) = process_image_to_palette(&img, &config) {
